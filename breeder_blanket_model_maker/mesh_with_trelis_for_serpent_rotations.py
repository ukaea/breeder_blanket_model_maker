...
def move_um_mesh_files(output_dir):
  for file in ['matfile', 'boundary', 'faces', 'neighbour', 'owner','detector_p','detector_n',['points']]+['points_angle_'+str(angle) for angle in range(0,360,10)]:
...
for angle in range(0,360,10):
  cubit.cmd('rotate Volume all angle '+str(angle)+'  about Z include_merged ')
  cubit.cmd('export openfoam "' + os.path.join(output_dir, 'serpent_mesh') + '" overwrite')
  os.rename(os.path.join(output_dir,'serpent_mesh', 'constant', 'polyMesh', 'points'),os.path.join( output_dir,'points_'+'angle_'+str(angle)) )
